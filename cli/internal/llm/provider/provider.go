...
func NewProvider(providerName models.ModelProvider, opts ...ProviderClientOption) (Provider, error) {
	...
	case models.ProviderAnthropic:
		// OAuth tokens (sk-ant-o*) are blocked by Anthropic on the public API.
		// Route through Claude Code CLI instead, which authenticates legitimately.
		if IsOAuthToken(clientOptions.apiKey) && HasClaudeCodeCLI() {
	...
	case models.ProviderOpenAI:
		// Route through Codex CLI when using ChatGPT OAuth token (JWT)
		if IsCodexCLIToken(clientOptions.apiKey) && HasCodexCLI() {
			return &baseProvider[ProviderClient]{
				options: clientOptions,
				client:  newCodexCodeClient(clientOptions),
			}, nil
		}
