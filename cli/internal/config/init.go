...
// ShouldShowWelcome checks if the welcome dialog should be shown (first ever run).
func ShouldShowWelcome() bool {
	if cfg == nil {
		return false
	}
	flagFilePath := filepath.Join(cfg.Data.Directory, "welcome")
	_, err := os.Stat(flagFilePath)
	return os.IsNotExist(err)
}

// MarkWelcomeShown marks the welcome dialog as shown.
func MarkWelcomeShown() error {
	...
	if err != nil {
		return fmt.Errorf("failed to create welcome flag file: %w", err)
	}
	defer file.Close()
	return nil
}
