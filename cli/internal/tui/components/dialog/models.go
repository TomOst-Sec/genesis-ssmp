...
type ModelDialog interface {
	tea.Model
	layout.Bindings
	SetProvider(provider models.ModelProvider)
	SetProviderFilter(providers []models.ModelProvider)
}
...
func (m *modelDialogCmp) View() string {
	...
	if m.provider == "cloud" {
		dialogWidth = maxDialogWidthWide
	}

	// Title based on provider filter mode
	var titleText string
	switch m.provider {
	case "cloud":
		titleText = "Select Angel Model"
	case models.ProviderLocal:
		titleText = "Select God Model"
	default:
	...
	for i := m.scrollOffset; i < endIdx; i++ {
		...
		// In combined mode, show provider tag
		if m.provider == "cloud" || m.provider == models.ProviderLocal {
			if !m.hScrollPossible && m.models[i].Provider != "" {
				tag := strings.ToUpper(string(m.models[i].Provider)[:1]) + string(m.models[i].Provider[1:])
				label = fmt.Sprintf("%-22s %s", m.models[i].Name, tag)
			}
		}
		modelItems = append(modelItems, itemStyle.Render(label))
	}

	scrollIndicator := m.getScrollIndicators(dialogWidth)

	content := lipgloss.JoinVertical(
		lipgloss.Left,
		title,
		baseStyle.Width(dialogWidth).Render(lipgloss.JoinVertical(lipgloss.Left, modelItems...)),
		scrollIndicator,
	)
...
// SetProvider forces the dialog to show models for a specific provider.
func (m *modelDialogCmp) SetProvider(provider models.ModelProvider) {
	m.setupModels()
	idx := findProviderIndex(m.availableProviders, provider)
	if idx >= 0 {
		m.hScrollOffset = idx
		m.provider = provider
		m.setupModelsForProvider(provider)
	}
}

// SetProviderFilter restricts the dialog to show all models from the given
// providers in a single flat list (no horizontal scrolling between tabs).
func (m *modelDialogCmp) SetProviderFilter(providers []models.ModelProvider) {
	m.setupModels()

	// Collect all models from matching enabled providers
	var allModels []models.Model
	var matchedProviders []models.ModelProvider
	for _, p := range m.availableProviders {
		for _, f := range providers {
			if p == f {
				matchedProviders = append(matchedProviders, p)
				allModels = append(allModels, getModelsForProvider(p)...)
				break
			}
		}
	}
	...
	// Sort combined list: reverse alphabetical by name
	...
	m.models = allModels
	m.availableProviders = matchedProviders
	m.hScrollPossible = false
	m.hScrollOffset = 0
	m.selectedIdx = 0
	m.scrollOffset = 0
	m.provider = "cloud" // label for the title
}
