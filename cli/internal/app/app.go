...
func New(ctx context.Context, conn *sql.DB) (*App, error) {
	...
	if config.NeedsAuth() {
		// No provider configured — use stub agent so TUI can boot.
		// Auth dialog will appear and call InitCoderAgent() after.
		app.CoderAgent = agent.NewStubAgent()
	} else {
...
// InitCoderAgent creates the coder agent with the current provider configuration.
// Called at startup if a provider is configured, or after auth dialog completion.
