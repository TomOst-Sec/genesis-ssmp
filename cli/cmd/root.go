...
import (
	...
	"github.com/genesis-ssmp/genesis/cli/internal/db"
	"github.com/genesis-ssmp/genesis/cli/internal/format"
	...
	"github.com/genesis-ssmp/genesis/cli/internal/tui"
...
var rootCmd = &cobra.Command{
	Use:   "genesis",
	Short: "Genesis SSMP — AI-powered software development terminal",
	Long: `Genesis is the interactive TUI for the Genesis SSMP (Sovereign Software Manufacturing Plant).
It provides an AI-assisted chat interface with swarm orchestration, code analysis, LSP integration,
and Heaven/God API connectivity for mission planning and execution.`,
	Example: `
  ...
  genesis -d -c /path/to/project

  # Print version
  genesis -v

  # Run a single non-interactive prompt
  genesis -p "Explain the use of context in Go"

  # Run a single non-interactive prompt with JSON output format
  genesis -p "Explain the use of context in Go" -f json
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cmd.Flag("version").Changed {
			fmt.Println(brand.RenderLogoWithVersion())
			return nil
		}
		...
		// Headless RunMission mode (no TUI required)
		runMission, _ := cmd.Flags().GetBool("run-mission")
		if runMission {
			taskDesc, err := genesis.ReadMissionFile(cwd)
			if err != nil {
				return fmt.Errorf("run-mission: %w (create MISSION.md or TEST_PROMPT.md in CWD)", err)
			}
			startTime := time.Now()
			runner := genesis.NewMissionRunner(cwd)
			result, err := runner.Run(taskDesc)
			elapsed := time.Since(startTime)
			if err != nil {
				return fmt.Errorf("run-mission failed: %w", err)
			}
			fmt.Println(genesis.FormatMissionResult(result))
...
func init() {
	...
	// Add run-mission flag for headless Genesis RunMission execution
	rootCmd.Flags().Bool("run-mission", false, "Run a Genesis mission from MISSION.md/TEST_PROMPT.md without TUI")
